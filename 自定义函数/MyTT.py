...
def HHV(S, N):
    if isinstance(N, int):
        if N == 0:
            ...
            return pd.Series(S).rolling(N).max().values
    elif isinstance(N, pd.Series) or isinstance(N, np.ndarray):
        return rolling_max(S, N)
    else:
        raise ValueError("N应该是一个整数或数据列")
...
def rolling_max(S, N):
    ...
    for i in range(len(S)):
        n = int(N[i]) if i < len(N) else 0
        if n > 0:
            # 获取窗口起始位置
            start = max(0, i - n + 1)
            result[i] = np.max(S[start:i + 1])

    return pd.Series(result, index=pd.RangeIndex(len(S)))
...
def GET_MACD(CLOSE, SHORT=12, LONG=26, M=9):  # EMA的关系，S取120日，和雪球小数点2位相同
